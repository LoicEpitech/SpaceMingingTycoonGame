...
public class PlanetTest {
    ...
    // --- Test 7 : Test du callback onEnterPlanet ---
    @Test
    void testOnEnterPlanetCallback() {
        Planet p = new Planet("Callisto", 1, 1, MoonType.FARM);

        final boolean[] triggered = {false};

        p.setOnEnterPlanet(() -> triggered[0] = true);

        // On ne peut pas tester audio/rover, mais on peut tester le callback
        p.enterPlanetView();

        assertTrue(triggered[0], "Le callback onEnterPlanet doit être déclenché");
    }
